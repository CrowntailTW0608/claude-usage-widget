...
- **`claude_usage.py`** — CLI 版本，直接在終端機輸出 Claude Pro 用量
...
`SESSION_KEY` 需從瀏覽器 DevTools 的 Cookie 中取得，屬於短效憑證，過期需重新取得。`ORG_ID` 固定，已在 `claude_usage.py` 中設有預設值。
...
- `_refresh()` — 每 60 秒觸發一次，在背景執行緒同時呼叫：
  ...
  - `_fetch_incidents()` → `_apply_incidents()` 更新事故區塊
